class AngbandDb < ActiveRecord::Base
    ...
    def self.getEvent(id, timezone)
        ...
        objs_rows = connection.select_all("select o.id, o.name from object o, event_object eo where eo.event_id = #{ret["id"]} and eo.object_id = o.id")
        objs = []
    ...
    def self.getUnknownObjects(objs)
        ...
        rows = connection.select_all("select name from object where name in (#{in_clause}) and status = 'N'")
        res = objs.clone
    ...
    def self.getObjectNamesAndIDs(objs)
        ...
        in_clause = objs.map { |x| sanitize(x) }.join(", ")
        sql = "select id, name from object where name in (#{in_clause}) and status = 'N'"
    ...
    def self.getEventList(from, qty, timezone, filters)
        ...
        for row in rows
            event_id = row["id"]
            sql = "select o.id, o.name, o.description from object o, event_object eo where eo.event_id = #{event_id} and o.id = eo.object_id"
    ...
    def self.getObjectList(from, qty, timezone)
        sql = "select o.id, o.name, o.description, o.creator, cr.name as cr_name, o.cr_date at time zone #{sanitize(timezone)} as cr_date, o.updater, up.name as up_name, o.up_date at time zone #{sanitize(timezone)} as up_date
                                      ...
                                      o.status = 'N' and
                                      o.creator = cr.id and
                                      o.updater = up.id order by o.cr_date asc "
        if qty > 0
            sql = sql + "limit #{sanitize(qty)} "
        end
